...
<div class="container">
	...
	<script>
		...
		var get_messages_loop = function() { 
			...
			//	friend_id = (this.url).substr((this.url).lastIndexOf('=')+1);
				for(var i=0; i<obj.length; i++){
					$('.view-body').append("<div class='look'><div class='bubble me'>" + obj[i].msg+"</div><input type='hidden' name='timestamp' value='" + obj[i].timestamp + "'</div>");
			}
			...
			//	console.log(result);
				...
				for(var i=0; i<obj.length; i++){
					if(obj[i].from != friend_id) { 
						$('.view-body').append("<div class='look'><div class='bubble you'>" + obj[i].msg+"</div></div>");
					} else { 
						$('.view-body').append("<div class='look'><div class='bubble me'>" + obj[i].msg+"</div><input type='hidden' name='timestamp' value='" + obj[i].timestamp + "'</div>");
					}
				}
		...
		$(document).on('click', '.send', function(){
			...
			$('.view-body').append("<div class='look'><div class='bubble you'>" +text+"</div></div>");
